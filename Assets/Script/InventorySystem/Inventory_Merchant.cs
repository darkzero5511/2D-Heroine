...
public class Inventory_Merchant : Inventory_Base
{
    ...
    public void FillShopList()
    {
        ...
        foreach (var shopList in shopData)
        {
            foreach (var itemData in shopList.itemList)
            {
                int randomizedStack = Random.Range(
                    itemData.minStackSizeAtShop,
                    itemData.maxStackSizeAtShop + 1
                );

                int finalStack = Mathf.Clamp(
                    randomizedStack,
                    1,
        ...
        int randomItemAmount = Random.Range(
            minItemsAmount,
            maxInventorySize + 1
        );

        int finalAmount = Mathf.Clamp(
            randomItemAmount,
            1,
            possibleItems.Count
        );

        for (int i = 0; i < finalAmount && possibleItems.Count > 0; i++)
        {
